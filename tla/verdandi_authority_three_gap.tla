...
CONSTANTS
  ...
  MaxTick,       \* 5 — max clock ticks before the model terminates
  MaxDelegationDepth,  \* 3
  MaxRequests,   \* 3 — bound on total action requests (state space control)
  RootCapID,     \* c0 — the beekeeper root cap
  NullCap,       \* sentinel for "no parent cap" — set to "null" (a string)
  BeekeeperID,   \* model value for the beekeeper actor (= beekeeper in cfg)
  PropertyOwnerID, \* model value for property_owner actor
  SupervisorRing,  \* model value for ℍ-ring (= supervisor_ring in cfg)
  UserRing,        \* model value for ℂ-ring (= user_ring in cfg)
  NoHolder         \* sentinel for unissued cap holder (= "noholder" string in cfg)

\* ============================================================
\* RING ORDERING: SupervisorRing > UserRing
\* Modelled as integers for easy comparison; constants resolve to model values.
\* ============================================================
RingValue(r) ==
  IF r = SupervisorRing THEN 2
  ELSE IF r = UserRing  THEN 1
...
InitCaps ==
  [ c \in CapIDs |->
      ...
      ELSE [ ring         |-> UserRing,
             scope        |-> "unissued",
             lifetime_end |-> 0,
             revoked      |-> TRUE,    \* unissued caps — revoked by convention
             parent_cap   |-> NullCap,
             holder       |-> NoHolder ] ]

\* Initial consents: not yet active, subject is PropertyOwnerID
InitConsents ==
  [ consent \in ConsentIDs |->
      [ subject  |-> PropertyOwnerID,
        active   |-> FALSE,
        expiry   |-> MaxTick + 10,
        revoked  |-> FALSE ] ]
...
\* The presented cap must have ring >= required_ring.
\* Gap2 checks only ring authority; Gap3 checks validity (revoked/expired).
\* This preserves three-gap independence: Gap3 can fail (cap revoked) while
\* Gap2 passes (ring was correct), and vice versa.

Gap2Check(cap_id, required_ring) ==
...
ThreeGapApproved(req) ==
  ...
  /\ Gap2Check(req.cap_id, req.required_ring)
...
ConsentRevoke(consent_id) ==
  ...
  /\ action_outcomes' = [ n \in DOMAIN action_outcomes |->
         IF \E req \in action_requests :
              /\ req.id = n
              /\ req.requires_gap1 = consent_id
              /\ action_outcomes[n] \in {"Pending", "Approved"}
         THEN "Denied"
         ELSE action_outcomes[n] ]
  /\ UNCHANGED <<caps, now, action_requests, next_req_id>>
...
GrantCap(delegator, delegate, new_cap_id, new_ring, new_lifetime, parent_cap_id) ==
  ...
  /\ caps[new_cap_id].holder = NoHolder   \* cap slot must be unissued
  ...
  /\ LET to_revoke == RevokedSet(cap_id)
     IN /\ caps' = [ c \in CapIDs |->
        ...
        /\ action_outcomes' = [ n \in DOMAIN action_outcomes |->
                 IF \E req \in action_requests :
                      /\ req.id = n
                      /\ req.cap_id \in to_revoke
                      /\ action_outcomes[n] \in {"Pending", "Approved"}
                 THEN "Denied"
                 ELSE action_outcomes[n] ]
  /\ UNCHANGED <<consents, now, action_requests, next_req_id>>

\* ---- Tick: advance the clock ----
\* When time advances, any Approved/Pending outcome whose temporal chain
\* would fail Gap3Check at the new time is retroactively Denied.
Tick ==
  /\ now < MaxTick
  /\ now' = now + 1
  /\ LET next_now == now + 1
         CapExpiresNext(cap_id) ==
           caps[cap_id].lifetime_end =< next_now
         ParentExpiresNext(cap_id) ==
           caps[cap_id].parent_cap # NullCap
           /\ caps[caps[cap_id].parent_cap].lifetime_end =< next_now
         ConsentExpiresNext(consent_id) ==
           consent_id # "none"
           /\ consent_id \in ConsentIDs
           /\ consents[consent_id].expiry =< next_now
     IN action_outcomes' = [ n \in DOMAIN action_outcomes |->
            IF \E req \in action_requests :
                 /\ req.id = n
                 /\ action_outcomes[n] \in {"Pending", "Approved"}
                 /\ \/ CapExpiresNext(req.cap_id)
                    \/ ParentExpiresNext(req.cap_id)
                    \/ ConsentExpiresNext(req.requires_gap1)
            THEN "Denied"
            ELSE action_outcomes[n] ]
  /\ UNCHANGED <<caps, consents, action_requests, next_req_id>>
