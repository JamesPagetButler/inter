...
| 2026-09-19 | Sprint 3 | federation (`confluent-trust#104` work) | `inter#117` → this PR | cth-implementor referenced PRs/issues by bare `#N` (`#104`, `#102`, `#103`) in beekeeper-facing terminal reports instead of `owner/repo#N`, despite the standing ruling (memory `state-repo-for-issue-pr-refs`, beekeeper 2026-09-03) which already scopes itself to "channel posts, PR comments, and **reports** alike." Beekeeper caught it and directed this filing; first mis-filed as a per-repo issue `confluent-trust#105` (closed) — the process for process faults is this ledger, not a per-repo issue. | one-off (proposed) — adherence lapse; the rule is already codified (name the repo), so the fix is enforcement not a new rule. Flag for retro: it recurred repeatedly within one session, so **systemic-enforcement** may be the truer classification. | Memory `state-repo-for-issue-pr-refs` strengthened — recurrence recorded + a hard pre-send check (scan drafted output for `#<number>`; prefix each first occurrence with `owner/repo`; treat a bare `#N` as a defect to fix before sending). Retro to confirm classification. |
